...
PEDIA_IMAGES = {
    "Красная Птица": ('bird_imgs', 0), "Взрывная Птица": ('bird_imgs', 1), "Ускоряющаяся Птица": ('bird_imgs', 2),
    "Птица-Дробилка": ('bird_imgs', 3), "Птица-Бумеранг": ('bird_imgs', 4), "Свинья-Мишень": 'target_img', "Кирпич": 'brick_img',
    "Red Bird": ('bird_imgs', 0), "Explosive Bird": ('bird_imgs', 1), "Speed-Up Bird": ('bird_imgs', 2),
    "Splitter Bird": ('bird_imgs', 3), "Boomerang Bird": ('bird_imgs', 4), "Target Pig": 'target_img', "Brick": 'brick_img'
}
...
def draw_campaign_board(screen, images, fonts, game_state, mx, my, texts):
    ...
    if board:
        for r in range(CAMPAIGN_GRID_SIZE):
            for c in range(CAMPAIGN_GRID_SIZE):
                if (r, c) in animated_grid_positions: continue
                ...
                if bird_index is not None:
                    ...
                    if game_state.get('campaign_board_state') == 'clearing' and (r,c) in game_state['campaign_matched_tiles']:
                    ...
                    if alpha > 0:
                        ...
                        if scaled_size > 0:
                            scaled_bird = pygame.transform.scale(bird_img, (scaled_size, scaled_size))
                            scaled_bird.set_alpha(alpha)
                            img_rect = scaled_bird.get_rect(center=(c * cell_size + cell_size / 2, r * cell_size + cell_size / 2))
                            board_surface.blit(scaled_bird, img_rect)

    def draw_animated_tile_at(bird_type, center_x, center_y, alpha=255):
        img = pygame.transform.scale(images['bird_imgs'][bird_type], (int(cell_size*0.9), int(cell_size*0.9)))
        img.set_alpha(alpha)
    ...
    if game_state.get('campaign_is_swapping'):
        ...
        curr_x1 = x1 + (x2 - x1) * p; curr_y1 = y1 + (y2 - y1) * p
        curr_x2 = x2 + (x1 - x2) * p; curr_y2 = y2 + (y1 - y2) * p
    ...
    if game_state.get('campaign_is_dragging_tile'):
        ...
        if bird_type is not None:
            ...
            draw_animated_tile_at(bird_type, c * cell_size + cell_size/2, r * cell_size + cell_size/2, alpha=100)
    ...
    def get_animated_y(tile_data, is_refilling=False):
        ...
        y_start = ((r_end + tile_data['start_y_offset']) if is_refilling else tile_data['start_pos'][0] * cell_size) + cell_size / 2
    ...
    if game_state.get('campaign_board_state') == 'falling':
        for tile in game_state['campaign_falling_tiles']:
            draw_animated_tile_at(tile['type'], tile['end_pos'][1] * cell_size + cell_size / 2, get_animated_y(tile))
    if game_state.get('campaign_board_state') == 'refilling':
        for tile in game_state['campaign_refilling_tiles']:
            draw_animated_tile_at(tile['type'], tile['end_pos'][1] * cell_size + cell_size / 2, get_animated_y(tile, True))

    if game_state.get('campaign_selected_tile') and not game_state.get('campaign_is_swapping'):
        r, c = game_state['campaign_selected_tile']
        pygame.draw.rect(board_surface, (255, 255, 0, 200), (c * cell_size, r * cell_size, cell_size, cell_size), 4, border_radius=5)
    ...
    if game_state['sound_on']: screen.blit(images['speaker_on_img'], (game_state['WIDTH'] - 50, 10))
    else: screen.blit(images['speaker_off_img'], (game_state['WIDTH'] - 50, 10))
    if game_state['paused']: screen.blit(images['resume_img'], (game_state['WIDTH'] - 100, 10))
    else: screen.blit(images['pause_img'], (game_state['WIDTH'] - 100, 10))
    screen.blit(images['lightbulb_img'], (game_state['WIDTH'] // 2 - int(30*game_state['scale_factor']), int(10*game_state['scale_factor'])))
    ...
    if game_state['campaign_level_complete']:
        ...
        win_surf, win_rect = draw_text(get_text(texts, 'campaign_win'), fonts['font'], (255, 215, 0))
        win_rect.center = (game_state['WIDTH'] // 2, game_state['HEIGHT'] // 2 - 50)
        screen.blit(win_surf, win_rect)
        buttons = {}
        restart_surf, restart_btn = draw_text(get_text(texts, 'training_restart'), fonts['small_font'], (255, 255, 255))
        restart_btn.center = (game_state['WIDTH'] // 2, game_state['HEIGHT'] // 2 + 20)
        if restart_btn.collidepoint(mx, my): restart_surf, _ = draw_text(get_text(texts, 'training_restart'), fonts['small_font'], (255, 200, 0))
        screen.blit(restart_surf, restart_btn)
        buttons['restart_btn'] = restart_btn
        exit_surf, exit_btn = draw_text(get_text(texts, 'training_exit_to_menu'), fonts['small_font'], (255, 255, 255))
        exit_btn.center = (game_state['WIDTH'] // 2, game_state['HEIGHT'] // 2 + 70)
        if exit_btn.collidepoint(mx, my): exit_surf, _ = draw_text(get_text(texts, 'training_exit_to_menu'), fonts['small_font'], (255, 200, 0))
...
def draw_level_selection(screen, fonts, game_state, mx, my, texts):
    buttons = {'levels': {}}
    title_surf, title_rect = draw_text(get_text(texts, 'level_selection_title'), fonts['font'], (0,0,0))
    ...
    back_surf, back_btn = draw_text(get_text(texts, 'back'), fonts['small_font'], (0,0,0))
    back_btn.bottomleft = (20, game_state['HEIGHT'] - 20)
    if back_btn.collidepoint(mx, my):
        back_surf, _ = draw_text(get_text(texts, 'back'), fonts['small_font'], (255, 200, 0))
...
def draw_campaign_hint_popup(screen, fonts, game_state, mx, my, texts):
    ...
    title_surf, title_rect = draw_text(get_text(texts, 'campaign_hint_title'), fonts['small_font'], (255, 215, 0))
    ...
    close_surf, close_btn = draw_text(get_text(texts, 'hint_popup_close'), fonts['small_font'], (255, 255, 255))
    close_btn.center = (dialog_rect.centerx, dialog_rect.bottom - 40)
    if close_btn.collidepoint(mx, my):
        close_surf, _ = draw_text(get_text(texts, 'hint_popup_close'), fonts['small_font'], (120, 255, 120))
...
def draw_menu(screen, images, fonts, game_state, mx, my, texts):
    ...
    button_keys = ['start_btn', 'profile_btn', 'modes_btn', 'set_btn', 'achievements_btn']
    button_text_keys = ["start_game", "profile", "modes_difficulty", "settings", "achievements"]
    hover_bird_size = int(40 * game_state['scale_factor'])
    bird_hover_images = [pygame.transform.scale(img, (hover_bird_size, hover_bird_size)) for img in images['bird_imgs']]
    ...
    for i, text_key in enumerate(button_text_keys):
        ...
        if text_rect.collidepoint(mx, my):
            ...
            screen.blit(bird_img, (text_rect.left - (hover_bird_size + 15), text_rect.centery - hover_bird_size // 2))
        screen.blit(text_surf, text_rect)
        buttons[button_keys[i]] = text_rect

    exit_surf, exit_btn = draw_text(get_text(texts, 'exit'), fonts['small_font'], (0,0,0))
    exit_btn.bottomright = (game_state['WIDTH'] - 20, game_state['HEIGHT'] - 20)
    if exit_btn.collidepoint(mx, my):
        exit_surf, _ = draw_text(get_text(texts, 'exit'), fonts['small_font'], (255, 200, 0))
    screen.blit(exit_surf, exit_btn)
    buttons['exit_btn'] = exit_btn

    pedia_surf, pedia_btn = draw_text(get_text(texts, 'birdpedia'), fonts['small_font'], (0,0,0))
    pedia_btn.bottomleft = (20, game_state['HEIGHT'] - 20)
    if pedia_btn.collidepoint(mx, my):
        pedia_surf, _ = draw_text(get_text(texts, 'birdpedia'), fonts['small_font'], (255, 200, 0))
    screen.blit(pedia_surf, pedia_btn)
    buttons['birdpedia_btn'] = pedia_btn
    
    profile_text_surf, _ = draw_text(f"{get_text(texts, 'profile_colon')} {game_state['current_profile']}", fonts['info_font'], (0,0,0))
...
def draw_profile_selection(screen, fonts, game_state, mx, my, texts):
    ...
    create_surf, create_btn = draw_text(get_text(texts, 'create'), fonts['small_font'], (0,0,0))
    create_btn.topleft = (input_box_rect.right + 20, input_box_rect.y)
    if create_btn.collidepoint(mx, my):
        create_surf, _ = draw_text(get_text(texts, 'create'), fonts['small_font'], (0, 180, 0))
    ...
    button_text = get_text(texts, 'exit') if is_initial_selection else get_text(texts, 'back')
...
def draw_profile_delete_confirmation(screen, fonts, game_state, mx, my, texts):
    ...
    question_surf, question_rect = draw_text(question_text, fonts['small_font'], (255, 255, 255))
    question_rect.centerx = dialog_rect.centerx
    question_rect.y = dialog_rect.y + 40
    screen.blit(question_surf, question_rect)
    yes_surf, yes_btn = draw_text(get_text(texts, 'yes'), fonts['small_font'], (200, 80, 80))
    yes_btn.center = (dialog_rect.centerx - 80, dialog_rect.centery + 40)
    if yes_btn.collidepoint(mx, my):
        yes_surf, _ = draw_text(get_text(texts, 'yes'), fonts['small_font'], (255, 120, 120))
    screen.blit(yes_surf, yes_btn)
    no_surf, no_btn = draw_text(get_text(texts, 'no'), fonts['small_font'], (80, 200, 80))
    no_btn.center = (dialog_rect.centerx + 80, dialog_rect.centery + 40)
    if no_btn.collidepoint(mx, my):
        no_surf, _ = draw_text(get_text(texts, 'no'), fonts['small_font'], (120, 255, 120))
...
def draw_sound_settings(screen, images, fonts, game_state, mx, my, texts):
    ...
    start_y = 280
    y_step = 80
    music_vol_surf, _ = draw_text(get_text(texts, 'music_volume'), fonts['small_font'], (0,0,0))
    ...
    pygame.draw.rect(screen, (0, 150, 0), (music_slider.x, music_slider.y, music_knob_x - music_slider.x, music_slider.height))
    pygame.draw.circle(screen, (0, 100, 0), music_knob_center, music_knob_radius)
    buttons['music_slider'] = music_slider

    sfx_vol_surf, _ = draw_text(get_text(texts, 'sfx_volume'), fonts['small_font'], (0,0,0))
    screen.blit(sfx_vol_surf, (game_state['WIDTH'] // 2 - 150, start_y + y_step))
    sfx_slider = pygame.Rect(game_state['WIDTH'] // 2 - 150, start_y + y_step + 40, 300, 10)
    sfx_knob_radius = 10
    ...
    pygame.draw.rect(screen, (0, 150, 0), (sfx_slider.x, sfx_slider.y, sfx_knob_x - sfx_slider.x, sfx_slider.height))
    ...
    prev_surf, prev_btn = draw_text(get_text(texts, 'prev_track'), fonts['small_font'], (0,0,0))
    prev_btn.topright = (track_rect.left - 20, track_rect.top)
    if prev_btn.collidepoint(mx, my):
        prev_surf, _ = draw_text(get_text(texts, 'prev_track'), fonts['small_font'], (255, 200, 0))
    screen.blit(prev_surf, prev_btn)
    buttons['prev_track_btn'] = prev_btn
    next_surf, next_btn = draw_text(get_text(texts, 'next_track'), fonts['small_font'], (0,0,0))
    next_btn.topleft = (track_rect.right + 20, track_rect.top)
    if next_btn.collidepoint(mx, my):
        next_surf, _ = draw_text(get_text(texts, 'next_track'), fonts['small_font'], (255, 200, 0))
...
def draw_screen_settings(screen, fonts, game_state, mx, my, texts):
    ...
    bright_text_surf, _ = draw_text(get_text(texts, 'brightness'), fonts['small_font'], (0, 0, 0))
    screen.blit(bright_text_surf, (slider_x, start_y))
    brightness_slider = pygame.Rect(slider_x, start_y + 40, slider_width, 10)
    knob_radius = 10
    knob_x = brightness_slider.x + int(game_state['brightness_slider_pos'] * brightness_slider.width)
    knob_center = (knob_x, brightness_slider.centery)
    pygame.draw.rect(screen, (150, 150, 150), brightness_slider)
    pygame.draw.rect(screen, (250, 250, 100), (brightness_slider.x, brightness_slider.y, knob_x - brightness_slider.x, brightness_slider.height))
    ...
    res_text_surf, _ = draw_text(get_text(texts, 'resolution'), fonts['small_font'], (0, 0, 0))
    ...
    res_fullscreen_surf, res_fullscreen_btn = draw_text(get_text(texts,"fullscreen"), fonts['small_font'], color_fullscreen)
    ...
    if not is_fullscreen_selected and res_fullscreen_btn.collidepoint(mx, my):
        res_fullscreen_surf, _ = draw_text(get_text(texts,"fullscreen"), fonts['small_font'], (255, 200, 0))
...
def draw_game_mode_selection(screen, images, fonts, game_state, mx, my, texts):
    ...
    draw_mode_button('sharpshooter', start_y + y_step, 'sharpshooter_btn', 'sharpshooter')
    ...
    if game_state['game_mode'] in modes_with_difficulty:
        ...
        knob_radius = 10
        if game_state['difficulty'] == 'easy': knob_pos = slider.x + knob_radius
        elif game_state['difficulty'] == 'medium': knob_pos = slider.centerx
        else: knob_pos = slider.right - knob_radius
        knob_center = (knob_pos, slider.centery)
        pygame.draw.circle(screen, (255, 0, 0), knob_center, knob_radius)
        
        easy_surf, _ = draw_text(get_text(texts, 'easy'), fonts['small_font'], (0,0,0))
        screen.blit(easy_surf, (slider.x, slider.y + 15))
        med_surf, _ = draw_text(get_text(texts, 'medium'), fonts['small_font'], (0,0,0))
        screen.blit(med_surf, (slider.centerx - med_surf.get_width()//2, slider.y + 15))
        hard_surf, _ = draw_text(get_text(texts, 'hard'), fonts['small_font'], (0,0,0))
...
def draw_achievements(screen, images, fonts, game_state, mx, my, texts):
    ...
    y_offset = 120
    profiles_title_surf, _ = draw_text(get_text(texts, 'profiles'), fonts['small_font'], (0,0,0))
    ...
    stats_title_surf, _ = draw_text(f"{get_text(texts, 'stats_for')} '{viewed_profile_name}':", fonts['small_font'], (0,0,0))
    screen.blit(stats_title_surf, (350, 150 + y_offset))

    difficulty_y = 200 + y_offset
    difficulties = {'easy': get_text(texts, 'easy'), 'medium': get_text(texts, 'medium'), 'hard': get_text(texts, 'hard')}
    current_x = 350
    ...
    classic_surf, _ = draw_text(f"{get_text(texts, 'max_combo_classic')} {score_classic}", stats_font, (0,0,0))
    ...
    ss_surf, _ = draw_text(f"{get_text(texts, 'max_combo_sharpshooter')} {score_ss}", stats_font, (0,0,0))
    ...
    obs_surf, _ = draw_text(f"{get_text(texts, 'max_combo_obstacle')} {score_obs}", stats_font, (0,0,0))
    screen.blit(obs_surf, (350, 350 + y_offset))
    
    reset_surf, reset_btn = draw_text(get_text(texts, 'reset_profile'), fonts['small_font'], (180, 0, 0))
    reset_btn.bottomright = (game_state['WIDTH'] - 20, game_state['HEIGHT'] - 20)
    if reset_btn.collidepoint(mx, my):
        reset_surf, _ = draw_text(get_text(texts, 'reset_profile'), fonts['small_font'], (255, 0, 0))
...
def draw_birdpedia_detail_screen(screen, fonts, game_state, mx, my, texts):
    ...
    description_text = pedia_descriptions.get(item_name, get_text(texts, 'pedia_not_found'))
...
def draw_hint_popup(screen, fonts, game_state, mx, my, texts):
    ...
    bird_name_ru = game_state.get('bird_image_to_name', {}).get(current_bird_img, "Неизвестная птица")
    
    try:
        bird_idx = list(game_state.get('bird_image_to_name', {}).values()).index(bird_name_ru)
        bird_name = get_text(texts, 'pedia_items')[bird_idx]
    except (ValueError, IndexError):
        bird_name = get_text(texts, 'unknown_bird')
    
    pedia_descriptions = get_text(texts, 'pedia_descriptions')
    description_text = pedia_descriptions.get(bird_name, get_text(texts, 'no_bird_on_slingshot'))
...
def draw_training_popup(screen, fonts, game_state, mx, my, texts):
    ...
    for word in words:
        ...
        screen.blit(word_surface, (x, y))
        x += word_width + space
    continue_surf, continue_btn = draw_text(get_text(texts, 'training_popup_continue'), fonts['small_font'], (255, 255, 255))
    continue_btn.center = (dialog_rect.centerx, dialog_rect.bottom - 40)
    if continue_btn.collidepoint(mx, my):
        continue_surf, _ = draw_text(get_text(texts, 'training_popup_continue'), fonts['small_font'], (120, 255, 120))
...
def draw_training_complete_screen(screen, fonts, game_state, mx, my, texts):
    ...
    overlay.fill((0, 0, 0, 180))
    screen.blit(overlay, (0, 0))
    title_surf, title_rect = draw_text(get_text(texts, 'training_complete_title'), fonts['font'], (255, 215, 0))
    title_rect.center = (game_state['WIDTH'] // 2, game_state['HEIGHT'] // 2 - 50)
    screen.blit(title_surf, title_rect)
    restart_surf, restart_btn = draw_text(get_text(texts, 'training_restart'), fonts['small_font'], (255, 255, 255))
    restart_btn.center = (game_state['WIDTH'] // 2, game_state['HEIGHT'] // 2 + 20)
    if restart_btn.collidepoint(mx, my):
        restart_surf, _ = draw_text(get_text(texts, 'training_restart'), fonts['small_font'], (255, 200, 0))
    screen.blit(restart_surf, restart_btn)
    buttons['restart_btn'] = restart_btn
    exit_surf, exit_btn = draw_text(get_text(texts, 'training_exit_to_menu'), fonts['small_font'], (255, 255, 255))
    exit_btn.center = (game_state['WIDTH'] // 2, game_state['HEIGHT'] // 2 + 70)
    if exit_btn.collidepoint(mx, my):
        exit_surf, _ = draw_text(get_text(texts, 'training_exit_to_menu'), fonts['small_font'], (255, 200, 0))
    screen.blit(exit_surf, exit_btn)
    buttons['exit_btn'] = exit_btn
    return buttons
...
def draw_gameplay(screen, images, fonts, game_state, texts):
    ...
    for i, bird_img in enumerate(game_state['bird_queue']):
        screen.blit(bird_img, (queue_start_x + i * queue_gap, GROUND_LEVEL - bird_size * 0.9))
    pygame.draw.circle(screen, (139, 69, 19), (game_state['sling_x'], game_state['sling_y']), int(5 * scale))
    ...
    if game_state['is_dragging'] and not game_state['paused']:
        ...
        pygame.draw.rect(screen, (100, 100, 100), (bar_x, bar_y, power_bar_width, power_bar_height))
        pygame.draw.rect(screen, (int(255 * power_percent), int(255 * (1 - power_percent)), 0), (bar_x, bar_y, int(power_bar_width * power_percent), power_bar_height))
        power_text_surf, _ = draw_text(f"{get_text(texts, 'power_colon')} {int(power_percent * 100)}%", fonts['small_font'], (0, 0, 0))
        screen.blit(power_text_surf, (game_state['sling_x'] - power_text_surf.get_width() // 2, bar_y + power_bar_height + 5))

    if game_state['show_rope']:
        pygame.draw.line(screen, (139, 69, 19), (game_state['sling_x'], game_state['sling_y']), (game_state['projectile_x'], game_state['projectile_y']), int(3 * scale))
    ...
    if game_state['game_over']:
        go_surf, go_rect = draw_text(get_text(texts, 'game_over'), fonts['font'], (255, 0, 0))

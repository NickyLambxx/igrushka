...
def load_images(width, height, ui_scale, game_scale):
    try:
        ...
        bird_imgs = [
            pygame.transform.scale(pygame.image.load(f"bird{i}.png").convert_alpha(), (bird_size, bird_size)) for i in range(1, 6)
        ]
        
        feather_imgs = [
            pygame.transform.scale(pygame.image.load(f"feather{i}.png").convert_alpha(), (int(20 * game_scale), int(20 * game_scale))) for i in range(1, 6)
        ]
        
        small_bird_img = pygame.transform.scale(pygame.image.load("bird4_small.png").convert_alpha(), (small_bird_size, small_bird_size))
        target_img = pygame.transform.scale(pygame.image.load("target.png").convert_alpha(), (bird_size, bird_size))
        
        target_defeated_img = pygame.transform.scale(pygame.image.load("target_defeated.png").convert_alpha(), (bird_size, bird_size))
        
        brick_img = pygame.transform.scale(pygame.image.load("brick.png").convert_alpha(), (bird_size, bird_size))
        
        speaker_on_img = pygame.transform.scale(pygame.image.load("speaker_on.png").convert_alpha(), (icon_size, icon_size))
        speaker_off_img = pygame.transform.scale(pygame.image.load("speaker_off.png").convert_alpha(), (icon_size, icon_size))
        pause_img = pygame.transform.scale(pygame.image.load("pause.png").convert_alpha(), (icon_size, icon_size))
        resume_img = pygame.transform.scale(pygame.image.load("play.png").convert_alpha(), (icon_size, icon_size))
        lightbulb_img = pygame.transform.scale(pygame.image.load("lightbulb.png").convert_alpha(), (lightbulb_w, lightbulb_h))
        
        smoke_img = pygame.transform.scale(pygame.image.load("smoke.png").convert_alpha(), (int(EXPLOSION_RADIUS * 2 * game_scale), int(EXPLOSION_RADIUS * 2 * game_scale)))
...
def load_sounds():
    try:
        ...
        return {
            'hit_sound': hit_sound, 'fly_sound': fly_sound, 'explosion_sound': explosion_sound,
            'boost_sound': boost_sound, 'split_sound': split_sound, 'brick_sound': brick_sound,
            'boomerang_sound': boomerang_sound, 'music_playlist': music_playlist
        }
    except Exception as e:

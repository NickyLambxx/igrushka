...
def create_trail_particle(trail_particles, x, y):
    trail_particles.append({
        'x': x, 'y': y, 'size': random.randint(3, 6),
        'life': random.randint(20, 30), 'color': (255, 165, 0)
    })
...
def create_dust_particle(dust_particles, x, y, count=1):
    ...
    for _ in range(count):
        dust_particles.append({
            'x': x, 'y': y, 'size': random.randint(5, 9),
            'life': random.randint(20, 30), 'dx': random.uniform(-2.5, 2.5),
            'dy': random.uniform(-2.5, 0), 'color': (200, 200, 200)
        })
...
def create_brick_shatter(dust_particles, x, y):
    ...
    for _ in range(15):
        dust_particles.append({
            'x': x, 'y': y, 'size': random.randint(3, 7),
...
def create_spark_particle(spark_particles, x, y):
    spark_particles.append({
        'x': x, 'y': y, 'size': random.randint(2, 4),
        'life': random.randint(10, 20), 'dx': random.uniform(-5, 5),
        'dy': random.uniform(-5, 5), 'color': (255, 255, 0)
    })
...
def create_feather_explosion(feather_particles, x, y, bird_index):
    for _ in range(15):
        ...
        feather_particles.append({
            ...
            'vy': math.sin(angle) * speed, 'angle': random.uniform(0, 360),
            'angular_velocity': random.uniform(-5, 5), 'life': random.randint(40, 60),
            'gravity': 0.1,
...
def draw_dashed_trajectory(screen, start_x, start_y, velocity_x, velocity_y, gravity, steps=50):
    points = []
    x, y = start_x, start_y
